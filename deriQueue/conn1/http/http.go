...
func request(res http.ResponseWriter, req *http.Request) {
	...
	fmt.Println("接收到请求" + req.URL.Path)
	var outStr []byte
...
func getData(url string, data public.UseType) (string, error) {
	var jsonData []byte
	jsonData, err := json.Marshal(data)
	if err != nil {
		return "", err
	}
	resData, err := http.Post(url, "application/json; charset=UTF-8", strings.NewReader(string(jsonData)))
